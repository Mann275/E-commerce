...
// Middleware that allows banned users - used for status checks
export const isAuthenticatedAllowBanned = async (req, res, next) => {
  try {
    ...
    // Don't block banned users - let them through
    req.user = user;
    req.id = user._id;
    next();
  } catch (error) {
