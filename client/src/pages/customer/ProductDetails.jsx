...
function ProductDetails() {
  ...
  const handleAddReview = async (e) => {
    ...
    try {
      const res = await apiClient.post(`/products/${id}/review`, {
        rating,
        comment,
      });
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-black transition-colors duration-300 pt-24 pb-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-8 overflow-x-auto whitespace-nowrap">
          ...
          <Link to="/products" className="hover:text-sky-500 transition-colors">
            Products
          </Link>
          <ChevronRight size={14} />
          <span className="hover:text-sky-500 transition-colors cursor-pointer">
            {product.category}
          </span>
          <ChevronRight size={14} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div className="grid grid-cols-4 gap-4">
              {displayImages.map((img, idx) => (
                <button
                  ...
                  className={`bg-white dark:bg-zinc-950 border rounded-2xl p-2 aspect-square flex items-center justify-center overflow-hidden transition-all duration-300 ${
                    activeImage === idx
          ...
          >
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <a
                href="#reviews"
                className="text-sm text-gray-500 hover:text-sky-500 underline decoration-dashed cursor-pointer transition-colors"
              >
            ...
            <div className="mb-6 p-6 bg-white dark:bg-zinc-950 border border-gray-200 dark:border-white/10 rounded-2xl shadow-sm">
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  <div
                    className={`w-2 h-2 rounded-full ${product.quantity > 0 ? "bg-emerald-500 animate-pulse" : "bg-rose-500"}`}
                  ></div>
                  {product.quantity > 0
                ...
                <span className="text-gray-400 text-sm">
                  SKU: OC-{product._id?.slice(-6).toUpperCase()}
                </span>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-10 pb-10 border-b border-gray-200 dark:border-white/10">
              ...
              <button
                onClick={() => {
                  ...
                  toast.success(
                    `${quantity}x ${product.productName} added to cart!`,
                  );
                }}
                disabled={product?.quantity <= 0}
                className={`flex-1 h-14 font-black rounded-xl flex items-center justify-center gap-2 transition-all relative overflow-hidden group ${
                  product?.quantity > 0
              ...
              >
                <span className="relative z-10 flex items-center gap-2 tracking-widest uppercase text-sm">
                  ...
                  {product?.quantity > 0 ? "Add to Cart" : "Item Unavailable"}
                </span>
                {product?.quantity > 0 && (
            ...
            <div className="bg-white dark:bg-zinc-950 border border-blue-400/30 dark:border-blue-500/30 rounded-2xl overflow-hidden shadow-sm p-6 mb-8">
              <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
                ...
                {product.userId && (
                  <Link
                    to={`/sellerinfo/${product.userId?._id || product.userId}`}
                    className="flex items-center gap-2 text-sm bg-sky-50 hover:bg-sky-100 text-sky-600 dark:bg-sky-500/10 dark:hover:bg-sky-500/20 dark:text-sky-400 font-bold px-4 py-2 rounded-xl transition-all shadow-sm"
                  >
        ...
        <div
          id="reviews"
          className="mt-20 border-t border-blue-400/30 dark:border-blue-500/30 pt-16"
        >
            ...
            <div className="lg:col-span-1">
              {isAuthenticated ? (
                ...
                <div className="bg-gray-50 dark:bg-white/5 p-8 rounded-3xl border border-gray-200 dark:border-white/10 text-center">
                  ...
                  <Link
                    to="/login"
                    className="inline-block px-6 py-2 bg-sky-500 text-white font-bold rounded-full transition-colors hover:bg-sky-600"
                  >
                    Login to Review
                  </Link>
                </div>
              ...
              {product.reviews && product.reviews.length > 0 ? (
                product.reviews
                  ...
                  .map((review, idx) => (
                    <div
                      key={idx}
                      className="bg-white dark:bg-zinc-950 p-6 rounded-2xl border border-gray-200 dark:border-white/10 shadow-sm flex flex-col sm:flex-row gap-4"
                    >
                      <div>
                ...
                <div className="text-center py-20 text-gray-500 dark:text-gray-400">
                  <Star
                    size={48}
                    className="mx-auto text-gray-300 dark:text-gray-700 mb-4 opacity-50"
                  />
