...
function Cart() {
  ...
  const handleUpdateQuantity = (id, newQuantity) => {
    ...
    const availableStock = currentItem?.stock || currentItem?.quantity || 0;
  ...
  const hasInvalidItems = cartItems.some(
    (item) => item.status === "inactive" || item.quantity <= 0,
  );
        ...
        {cartItems.length === 0 ? (
          <div className="text-center py-20 bg-white dark:bg-zinc-950 border border-blue-400/30 dark:border-blue-500/30 rounded-3xl shadow-sm">
            ...
            <p className="text-gray-500 dark:text-gray-400 mb-8 max-w-md mx-auto">
              Looks like you haven't added any premium components to your cart
              yet. Let's get you building!
            </p>
            <Link
              to="/products"
              ...
              <AnimatePresence>
                {cartItems.map((item) => (
                  ...
                  >
                    ...
                    <div className="w-full sm:w-32 aspect-square bg-gray-50 dark:bg-black rounded-xl p-2 shrink-0">
                      <img
                        src={
                    ...
                    <div className="flex-1 w-full text-center sm:text-left">
                      <div className="text-xs font-bold text-sky-500 mb-1 tracking-wider uppercase">
                        {item.category}
                      </div>
                      <Link
                        to={`/product/${item._id}`}
              ...
              <div className="bg-white dark:bg-zinc-950 border border-gray-200 dark:border-white/10 rounded-3xl p-6 md:p-8 sticky top-24 shadow-sm">
                ...
                >
                  <span className="relative z-10 flex items-center gap-2 text-sm font-bold">
                    {hasInvalidItems ? "Review Items" : "Proceed to Checkout"}{" "}
                    <ArrowRight size={20} />
                  </span>
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-3 text-sm text-gray-500 dark:text-gray-400">
                    <Lock size={18} className="text-emerald-500" /> Secure
                    Checkout
                  </div>
                  <div className="flex items-center gap-3 text-sm text-gray-500 dark:text-gray-400">
                    <ShieldCheck size={18} className="text-sky-500" /> Buyer
                    Protection Guarantee
                  </div>
                  <div className="flex items-center gap-3 text-sm text-gray-500 dark:text-gray-400">
                    <CreditCard size={18} className="text-indigo-500" /> All
