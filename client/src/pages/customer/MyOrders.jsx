...
function MyOrders() {
  ...
  if (loading)
    return (
      <div className="flex items-center justify-center min-h-screen text-sky-500 font-black animate-pulse uppercase tracking-[0.2em] italic text-xs">
  ...
  const filteredOrders = orders.filter((o) => {
    const matchesTab =
      activeTab === "All Orders" || o.orderStatus === activeTab;
    const matchesSearch =
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {filteredOrders.length === 0 ? (
          ...
          >
            <Package
              size={64}
              className="text-sky-500/50 mx-auto mb-6 drop-shadow-lg"
            />
            ...
            <Link
              to="/products"
              className="bg-linear-to-r from-sky-500 to-indigo-500 text-white px-8 py-4 rounded-xl font-bold hover:scale-105 hover:shadow-[0_0_20px_rgba(14,165,233,0.4)] transition-all inline-block shadow-lg"
            >
              Start Shopping
            </Link>
          </motion.div>
                  ...
                  <div className="p-6 md:p-8 relative overflow-hidden">
                    <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
                      <div className="flex items-center gap-5">
                        <div
                          className={`w-14 h-14 rounded-2xl ${config.bg} ${config.color} flex items-center justify-center shadow-inner`}
                        >
                          <config.icon size={24} />
                        </div>
                        <div>
                      ...
                      <div className="flex flex-wrap items-center gap-4 md:gap-8">
                        ...
                        <div>
                          <div className="text-[10px] font-black uppercase tracking-widest text-gray-500 dark:text-zinc-500 mb-1">
                            Total Amount
                          </div>
                          <div className="text-xl font-black text-transparent bg-clip-text bg-linear-to-r from-sky-400 to-indigo-500 italic shadow-sm">
                            ₹{order.totalAmount.toLocaleString()}
                          </div>
                        </div>
                        <span
                          className={`px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest ${config.bg} ${config.color}`}
                        >
                          {config.label}
                        </span>
                      </div>
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-gray-100 dark:border-white/5">
                      <div className="flex items-center gap-2">
                        {order.items.slice(0, 3).map((item, i) => (
                          <div
                            key={i}
                            className="w-12 h-12 rounded-xl bg-white dark:bg-zinc-900 border border-gray-100 dark:border-white/10 p-1 overflow-hidden shadow-sm group-hover:border-sky-500/30 transition-colors"
                          >
                            <img
                              src={
                            ...
                            <div className="bg-gray-50 dark:bg-black rounded-2xl p-6 space-y-4">
                              {order.items.map((item, i) => (
                                ...
                                >
                                  ...
                                  <div className="text-sm font-black dark:text-white italic">
                                    ₹
                                    {(
