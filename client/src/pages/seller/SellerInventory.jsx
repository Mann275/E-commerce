...
function SellerInventory() {
  ...
  const handleDelete = async (productId) => {
    ...
    try {
      const res = await apiClient.delete(`/products/delete/${productId}`);
    ...
    <div className="animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-500 dark:text-zinc-500 font-medium text-sm">
            Manage and track your listed components.
          </p>
        </div>
      ...
      <div className="bg-white dark:bg-zinc-950 border border-gray-200 dark:border-white/5 rounded-[2.5rem] overflow-hidden shadow-sm">
        <div className="p-4 border-b border-gray-100 dark:border-white/5 bg-gray-50/50 dark:bg-white/2 flex flex-wrap gap-4 items-center justify-between">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-gray-100 dark:border-white/5 bg-gray-50/30 dark:bg-white/1">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-white/5">
              {filteredProducts.length > 0 ? (
                filteredProducts.map((p) => (
                  <tr
                    key={p._id}
                    className={`group hover:bg-gray-50/50 dark:hover:bg-white/1 transition-colors ${p.status === "inactive" ? "opacity-60 grayscale-[0.5]" : ""}`}
                  >
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-lg overflow-hidden shrink-0 border border-gray-100 dark:border-white/5">
                          <img
                            src={
                              p.productImg?.[0]?.url ||
                    ...
                    <td className="px-6 py-4 text-center text-sm text-gray-500 dark:text-zinc-500">
                      {p.quantity}
                    </td>
                    <td className="px-6 py-4 text-center">
                      <span
                        className={`px-2 py-1 rounded-md text-[8px] font-black uppercase tracking-widest ${p.status === "inactive" ? "bg-rose-500/10 text-rose-500" : "bg-emerald-500/10 text-emerald-500"}`}
                      >
                ...
                <tr>
                  <td
                    colSpan="6"
                    className="px-6 py-12 text-center text-gray-500 dark:text-zinc-500 uppercase font-black tracking-widest text-xs"
                  >
