...
function EditProduct() {
  ...
  useEffect(() => {
    const fetchProductData = async () => {
      try {
        const res = await apiClient.get(`/products/get/${productId}`);
        if (res.data.success) {
  ...
  const addUrlImage = () => {
    ...
    if (images.length + existingImages.length + formData.imageUrls.length >= 6)
    ...
    <div className="max-w-5xl mx-auto animate-fade-in p-2">
      ...
      >
        ...
        <p className="text-gray-500 dark:text-zinc-500 font-medium max-w-xl">
          Update the technical configurations and marketplace data for your
          hardware.
        </p>
      </motion.div>
