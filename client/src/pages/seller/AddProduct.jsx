...
function AddProduct() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (images.length === 0 && formData.imageUrls.length === 0)
    ...
    try {
      ...
      const res = await apiClient.post(`/products/add-product`, data);
    ...
    <div className="max-w-5xl mx-auto animate-fade-in p-2">
      ...
      >
        ...
        <p className="text-gray-500 dark:text-zinc-500 font-medium max-w-xl">
          Deploy high-performance hardware to the OverClocked marketplace. Fill
          in the technical specifications below.
        </p>
      </motion.div>
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white dark:bg-zinc-950 border border-gray-100 dark:border-white/5 rounded-[2.5rem] p-8 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              ...
              {images.map((img, idx) => (
                <div
                  key={`file-${idx}`}
                  className="relative aspect-square rounded-2xl overflow-hidden group border border-gray-100 dark:border-white/5"
                >
                  <img
                    src={img.preview}
                    alt="preview"
              ...
              {formData.imageUrls.map((url, idx) => (
                <div
                  key={`url-${idx}`}
                  className="relative aspect-square rounded-2xl overflow-hidden group border border-sky-500/30"
                >
                  <img
                    src={url}
                    alt="url preview"
                    className="w-full h-full object-cover transition-transform group-hover:scale-110"
                  />
                  <div className="absolute top-1 left-1 px-1.5 py-0.5 bg-sky-500 text-[8px] font-black text-white rounded-md uppercase">
                    Link
                  </div>
                  <button
                    type="button"
              ...
              {images.length + formData.imageUrls.length < 6 && (
                <label className="aspect-square rounded-2xl border-2 border-dashed border-gray-200 dark:border-zinc-800 flex flex-col items-center justify-center cursor-pointer hover:border-sky-500 hover:bg-sky-500/5 transition-all group">
                  <Plus
                    ...
                    size={24}
                  />
                  <span className="text-[10px] font-black uppercase tracking-widest text-gray-400 group-hover:text-sky-500 mt-2">
                    Add Slot
                  </span>
                  <input
                    type="file"
          ...
          <div className="bg-gradient-to-br from-zinc-900 to-black p-8 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden group">
            <div className="relative z-10">
              <h3 className="text-lg font-black uppercase tracking-tighter italic mb-4">
                Seller Note
              </h3>
              <p className="text-xs text-zinc-400 leading-relaxed font-medium mb-6">
                Quality images increase conversion by 40%. Ensure your hardware
                is well-lit and showcased from multiple angles.
              </p>
              <div className="flex items-center gap-2 text-sky-400">
                <CheckCircle2 size={16} />
                <span className="text-[10px] font-black uppercase tracking-widest">
                  Verified Seller Platform
                </span>
              </div>
            </div>
